@echo off
rem ===========================================================
rem  URL Redirect Map (standalone) launcher
rem    1) make sure the Docker engine is up
rem    2) docker compose up -d --build  (backend + frontend)
rem    3) open the browser and exit
rem  Ports 5175 (frontend) / 8002 (backend) so it runs next to the main stack + other standalones.
...
title URL Redirect Map launcher
echo.
echo   ===== URL Redirect Map v0.1 (standalone) =====
echo.
...
if %errorlevel%==0 (
  ...
  call :waitdocker 60
  if not %errorlevel%==0 (
    echo  *** Docker could not be started. Start Docker Desktop, then run start-redirectmap.bat again. ***
    pause
    exit /b 1
  )
)
...
echo [3/3] Opening http://localhost:5175 ...
...
set /a _bt=0
:wb_loop
curl -fsS -m 2 -o nul http://127.0.0.1:8002/api/health >nul 2>&1
if %errorlevel%==0 exit /b 0
set /a _bt+=2
