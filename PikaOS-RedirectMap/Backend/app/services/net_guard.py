"""SSRF guard for outbound HTTP to user-supplied URLs (the redirect-map probe path).

The redirect-map tool probes arbitrary user-supplied URLs (the old + new sides of each
mapping row). Without a guard a user could aim it at internal hosts — cloud metadata
(169.254.169.254), `localhost`, RFC1918 ranges — so every outbound request is checked.
...
- `guarded_event_hooks()` — an httpx **request** event hook attached to every client in
  the probe path. httpx fires it for the initial request *and every redirect hop*, so
  ...
  is marked unreachable rather than crashing the whole run).
...
def assert_public_url(url: str) -> None:
    ...
    resolved IP into the connection is future hardening.
    """
    if not settings.redirect_ssrf_block_private:
        return
    ...
    allow = settings.redirect_allowlist
...
def guarded_event_hooks() -> dict:
    """`event_hooks` for httpx clients in the probe path (empty if the guard is off)."""
    if not settings.redirect_ssrf_block_private:
        return {}
